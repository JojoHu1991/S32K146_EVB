...
#define ADC_IP_SW_MAJOR_VERSION_C               3
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
#define ADC_START_SEC_CONST_UNSPECIFIED
#include "Adc_MemMap.h"

/* Mapping array of input channel to indexes. */
static const Adc_Ip_InputChannelType AdcMappingIndexToChannel[ADC_IP_INPUTCHAN_MAX_INDEX] = IP_MAP_INDEX_TO_CHANNEL;

#define ADC_STOP_SEC_CONST_UNSPECIFIED
#include "Adc_MemMap.h"
#endif /* (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA) */
/*==================================================================================================
*                                      LOCAL VARIABLES
==================================================================================================*/
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
    /* init all channel index mapping elements with invalid channel index */
    for (Index = 0u; Index < ADC_IP_INPUTCHAN_MAX_INDEX; Index++)
    {
        Adc_Ip_axState[Instance].ChannelConfigMapInputChannelToHardwareIndex[Index] = ADC_MAX_CHAN_COUNT;
    }
#endif /* (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA) */
...
#endif /* (ADC_IP_ENABLE_SIM_SOURCE_SELECTION == STD_ON) */
#if (ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
    Adc_Ip_axState[Instance].CalibrationOnGoing = FALSE;
                          ...
                          const Adc_Ip_ChanConfigType * const ChanConfig)
{
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
        InputChanDemapped = (Adc_Ip_InputChannelType)ADC_IP_INPUTCHAN_INT0_VAL;
#else
    ...
    {
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
        InputChanDemapped = AdcMappingIndexToChannel[ChanConfig->Channel];
#else
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
    if (ADC_IP_INPUTCHAN_DISABLED != ChanConfig->Channel)
    {
        /* Add the channel index to the coresponding channel input map */
        Adc_Ip_axState[Instance].ChannelConfigMapInputChannelToHardwareIndex[ChanConfig->Channel] = ChanConfig->ChnIdx;
    }
#else
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
    Adc_Ip_InputChannelType InputChannelTemp = AdcMappingIndexToChannel[InputChannel];
#else
    Adc_Ip_InputChannelType InputChannelTemp = InputChannel;
#endif /* (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA) */
...
#if (ADC_IP_AIEN_INTERRUPT_ENABLE == STD_ON)
    Adc_HwAcc_SetChannel(Base, 0u, InputChannelTemp, InterruptEnable);
#else
    Adc_HwAcc_SetChannel(Base, 0u, InputChannelTemp, FALSE);
...
Adc_Ip_StatusType Adc_Ip_DoCalibration(const uint32 Instance)
{
    ...
    uint32 Reg;
#if (ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
    static Adc_Ip_ClockConfigType PreClockConfig;
    static Adc_Ip_TrigType TriggerMode = ADC_IP_TRIGGER_SOFTWARE;
#else
...
#if (ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
    if (Adc_Ip_axState[Instance].CalibrationOnGoing == FALSE)
    {
        Adc_Ip_axState[Instance].CalibrationOnGoing = TRUE;
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
        ...
        SchM_Exit_Adc_ADC_EXCLUSIVE_AREA_29();
#if (ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
        Status = ADC_IP_STATUS_CAL_ONGOING;
    }
    else
#endif /*(ADC_SAR_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
    {
...
Adc_Ip_StatusType Adc_Ip_GetChanData(const uint32 Instance,
                                     ...
                                     uint16 * const Result)
{
#if (STD_ON == ADC_IP_DEV_ERROR_DETECT)
    DevAssert(Instance < ADC_INSTANCE_COUNT);
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
    if (ADC_IP_INPUTCHAN_DISABLED != Channel)
    {
        /* Add the channel index to the coresponding channel input map */
        ControlChanIdx = Adc_Ip_axState[Instance].ChannelConfigMapInputChannelToHardwareIndex[Channel];
    }
    else
    {
        ControlChanIdx = ADC_MAX_CHAN_COUNT;
    }
#else
    boolean Found = FALSE;
...
#if (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA)
    if (ADC_MAX_CHAN_COUNT > ControlChanIdx)
#else
    if (TRUE == Found)
#endif /* (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA) */
    {
...
/*FUNCTION**********************************************************************
*
* Function Name : Adc_Ip_SetUserGainAndOffset
* Description   : This function configures user gain and offset.
*
* @implements      Adc_Ip_SetUserGainAndOffset_Activity
* END**************************************************************************/
void Adc_Ip_SetUserGainAndOffset(const uint32 Instance,
                                 const uint16 Gain,
                                 const uint8 Offset)
{
#if (STD_ON == ADC_IP_DEV_ERROR_DETECT)
    ...
    ADC_Type * Base = Adc_Ip_apxBase[Instance];

    Adc_HwAcc_SetUserGainAndOffset(Base, Gain, Offset);
}
