...
#define ADC_IP_SW_MAJOR_VERSION_TYPES               3
...
/* Check if Adc_Ip_Types.h file and Std_Types.h file are of the same Autosar version */
    ...
    #error "AutoSar Version Numbers of Adc_Ip_Types.h and Std_Types.h are different"
#endif
...
typedef enum
{
    ...
    ADC_IP_STATUS_CAL_ONGOING = 0x03U    /*!< Calibration functionality is ongoing */
#endif /*(ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON) */
} Adc_Ip_StatusType;
...
#if (STD_OFF == ADC_IP_OPTIMIZE_GET_CONV_DATA)
/*!
...
typedef struct
{
    ...
    /* Current channel hardware config */
    uint8 ChannelConfigMapInputChannelToHardwareIndex[ADC_IP_INPUTCHAN_MAX_INDEX];  /*!< Mapping of input channels to the actual hw channel index */
#else
    /* Current hardware channel config */
    Adc_Ip_InputChannelType ChannelConfig[ADC_MAX_CHAN_COUNT];      /*!< Current hardware channel config */
#endif /* (STD_ON == ADC_IP_OPTIMIZE_GET_CONV_DATA) */
#if (ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)
    boolean CalibrationOnGoing;
#endif /* (ADC_IP_ASYNC_CALIBARTION_ENABLED == STD_ON)*/
